-- Criar tabelas necessárias no Supabase para o SITE administrativo - VERSÃO CORRIGIDA
...
-- Inserir organizações padrão (sem ON CONFLICT)
INSERT INTO organizacoes (nome, tipo, codigo, matriz_id) 
SELECT 'Matriz DNOTAS', 'matriz', 'matriz', null
WHERE NOT EXISTS (SELECT 1 FROM organizacoes WHERE codigo = 'matriz');

INSERT INTO organizacoes (nome, tipo, codigo, matriz_id) 
SELECT 'Filial 1', 'filial', 'filial01', 'matriz'
WHERE NOT EXISTS (SELECT 1 FROM organizacoes WHERE codigo = 'filial01');

INSERT INTO organizacoes (nome, tipo, codigo, matriz_id) 
SELECT 'Filial 2', 'filial', 'filial02', 'matriz'
WHERE NOT EXISTS (SELECT 1 FROM organizacoes WHERE codigo = 'filial02');
...
-- Inserir funcionários padrão (sem ON CONFLICT)
...
AND NOT EXISTS (SELECT 1 FROM funcionarios WHERE email = 'admin@dnotas.com');
...
FROM organizacoes org 
WHERE org.codigo = 'matriz'
AND NOT EXISTS (SELECT 1 FROM funcionarios WHERE email = 'gestor@dnotas.com');
