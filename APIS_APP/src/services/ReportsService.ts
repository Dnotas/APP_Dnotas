...
export class ReportsService {
  ...
  async criarRelatorioVendas(data: {
    ...
    const result = await this.db.query<RelatorioVendas>(query, params);
    const relatorio = result.rows[0];

    // Enviar notificação para o cliente
    await this.notificationService.enviarNotificacaoRelatorio(
      data.cliente_cnpj,
      data.filial_id,
      relatorio
    );
  ...
  async obterRelatoriosCliente(
    ...
    const [dataResult, countResult] = await Promise.all([
      this.db.query<RelatorioVendas>(query, params),
  ...
  async obterRelatorioHoje(cliente_cnpj: string, filial_id: string): Promise<RelatorioVendas | null> {
    const hoje = new Date();
    ...
    const result = await this.db.query<RelatorioVendas>(
      query, 
      [cliente_cnpj, filial_id, inicioDodia, fimDoDia]
    );

    return result.rows[0] || null;
  }

  async obterEstatisticasVendas(cliente_cnpj: string, filial_id: string, dias: number = 30): Promise<{
  ...
  async atualizarRelatorioVendas(
    id: string,
    cliente_cnpj: string,
    filial_id: string,
    data: Partial<RelatorioVendas>
  ): Promise<RelatorioVendas> {
    const allowedFields = [
    ...
    const values = Object.keys(data)
      .filter(key => allowedFields.includes(key))
      .map(key => data[key as keyof RelatorioVendas]);
  ...
  async excluirRelatorio(id: string, cliente_cnpj: string, filial_id: string): Promise<boolean> {
    const query = `
