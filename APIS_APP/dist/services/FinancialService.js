...
class FinancialService {
    ...
    `;
        ...
        await this.notificationService.enviarNotificacaoBoleto(data.cliente_cnpj, data.filial_id, boleto);
        return boleto;
    }
    async obterBoletosCliente(cliente_cnpj, filial_id, filters = {}) {
        const { page = 1, limit = 10, status, dataVencimentoInicio, dataVencimentoFim, sortBy = 'data_vencimento', sortOrder = 'ASC' } = filters;
    ...
    async obterBoletosPendentes(cliente_cnpj, filial_id) {
        const query = `
      SELECT * FROM boletos 
    ...
    async obterBoletosVencidos(cliente_cnpj, filial_id) {
        const hoje = new Date();
        const query = `
      UPDATE boletos 
      ...
      RETURNING *
    `;
        const result = await this.db.query(query, [cliente_cnpj, filial_id, hoje]);
        return result.rows;
    }
    async marcarBoletoPago(numero_boleto, cliente_cnpj, filial_id, data_pagamento) {
      ...
      RETURNING *
    `;
        const result = await this.db.query(query, [dataPagamento, numero_boleto, cliente_cnpj, filial_id]);
        if (result.rows.length === 0) {
            throw new Error('Boleto não encontrado ou já foi pago');
        }
        return result.rows[0];
    }
    async cancelarBoleto(numero_boleto, cliente_cnpj, filial_id) {
        const query = `
      UPDATE boletos 
      ...
      RETURNING *
    `;
        const result = await this.db.query(query, [numero_boleto, cliente_cnpj, filial_id]);
        if (result.rows.length === 0) {
            throw new Error('Boleto não encontrado ou não pode ser cancelado');
        }
        return result.rows[0];
    }
    async obterEstatisticasFinanceiras(cliente_cnpj, filial_id) {
        const query = `
      SELECT 
    ...
    async obterExtrato(cliente_cnpj, filial_id, dataInicio, dataFim) {
        let whereConditions = ['cliente_cnpj = $1', 'filial_id = $2'];
        let params = [cliente_cnpj, filial_id];
        let paramIndex = 3;
        if (dataInicio) {
    ...
    `;
        const [boletosResult, resumoResult] = await Promise.all([
            this.db.query(boletosQuery, params),
    ...
    async verificarBoletosVencendo() {
    ...
    `;
        ...
        for (const row of result.rows) {
            await this.notificationService.enviarNotificacaoVencimento(row.cliente_cnpj, row.filial_id, row.quantidade);
        }
    }
}
